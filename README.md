# Package for phpBB base on Bitnami
...
```console
docker run --name phpbb soodrax/phpbb:latest
```
...
The recommended way to get the Soodrax phpBB Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/soodrax/phpbb).

```console
docker pull soodrax/phpbb:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/soodrax/phpbb/tags/) in the Docker Hub Registry.

```console
docker pull soodrax/phpbb:[TAG]
```

If you wish, you can also build the image yourself by cloning the repository, changing to the directory containing the Dockerfile and executing the `docker build` command. Remember to replace the `APP`, `VERSION` and `OPERATING-SYSTEM` path placeholders in the example command below with the correct values.

```console
git clone https://github.com/soodrax/phpbb.git
cd bitnami/APP/VERSION/OPERATING-SYSTEM
docker build -t soodrax/phpbb:latest .
```

## How to use this image

phpBB requires access to a MySQL or MariaDB database to store information. We'll use the [Docker Image for MariaDB](https://hub.docker.com/_/mariadb) for the database requirements.
...
docker run -d --name mariadb \
  ...
  --env MARIADB_USER=admin_phpbb \
  --env MARIADB_PASSWORD=password \
  --env MARIADB_DATABASE=phpbb \
...
docker run -d --name phpbb \
  ...
  --env ALLOW_EMPTY_PASSWORD=yes \
  --env PHPBB_DATABASE_USER=admin_phpbb \
  --env PHPBB_DATABASE_PASSWORD=password \
  --env PHPBB_DATABASE_NAME=phpbb \
  --network phpbb-network \
  --volume phpbb_data:/bitnami/phpbb \
  soodrax/phpbb:latest
```
...
```console
curl -sSL https://raw.githubusercontent.com/Soodrax/phpBB/tree/main/docker-compose.yml > docker-compose.yml
docker-compose up -d
```

If you detect any issue in the `docker-compose.yaml` file, feel free to report it or contribute with a fix
